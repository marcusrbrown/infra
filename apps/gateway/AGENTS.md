...
The gateway is the Fro Bot Discord client and workspace runner — a 3-service Docker Compose stack (gateway daemon, workspace executor, mitmproxy egress filter) deployed on a dedicated DigitalOcean droplet at `gateway.fro.bot`. The upstream source is `fro-bot/agent`, pinned to `v0.44.2` in `apps/gateway/upstream.json`. (Held at v0.44.2: v0.46.1's runtime image crash-loops with `ERR_MODULE_NOT_FOUND` for `@fro-bot/runtime` — an upstream packaging bug where the compiled gateway resolves the runtime workspace package to its `src/` path, absent from the runtime image. The deploy's GitHub App secret materialization and `add-project` support are in place and will activate once a fixed v0.46.x image is verified to boot.) There is no public HTTP surface; the gateway connects outbound to Discord and S3 only. Management happens via SSH and the `bunx @marcusrbrown/infra gateway *` CLI commands.
