...
**Verification:**
- Renovate config validates. A subsequent Renovate run would not open an auto-mergeable PG≥16 PR (it would, at most, surface under dependency-dashboard approval).
...
**Approach:**
- The operator runs the Unit 2 runbook against the live droplet (staging validation → cutover → verify). Because the deploy script never recreates the volume, the cutover's volume handling is done by the runbook directly on the droplet, then the repo pin is updated so the *next* normal `umami deploy` is consistent with the running image.
- Land the repo changes only after live verification confirms PG18 healthy, so `main` never claims a state the droplet hasn't reached.
- Sequencing: the production migration (runbook) executes and is verified FIRST; the repo PR (pin + volume rename + AGENTS.md + ceiling lift) lands only AFTER live verification, so `main` never claims a state the droplet hasn't reached. The volume rename in this PR must exactly match the volume name the runbook created on the droplet. This documents an already-executed, already-verified migration — analogous to how upstream pin bumps land after live confirmation elsewhere in this repo.
