// ─── Broker CLI host validation ───────────────────────────────────────────────
//
// Validates BROKER_HOST / BROKER_DOMAIN values before they are passed as ssh
// argv arguments. A value starting with `-` would be interpreted by ssh as an
// option flag, enabling ProxyCommand injection and local code execution.
//
// This is the CLI-side validator. The deploy-side validator lives at
// apps/broker/src/host.ts. Both enforce the same rules.
