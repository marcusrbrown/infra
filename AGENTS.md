...
- **GitHub Actions**: `.yaml` extension (not `.yml`). SHA-pin all actions with a trailing version comment — either `# vX.Y.Z` or, for a scoped release tag, `# name@X.Y.Z` (e.g. `# renovate-changesets@0.2.48`). (enforced)
...
bunx @marcusrbrown/infra cliproxy monitor         # Live provider probe with issue/Discord alerts
bunx @marcusrbrown/infra cliproxy reset-quota     # Reset quota for a CLIProxyAPI auth record
...
- `DASHBOARD_SSH_KEY`, `DASHBOARD_DOMAIN`, `DASHBOARD_GITHUB_APP_ID`, `DASHBOARD_GITHUB_APP_KEY`, `DASHBOARD_OAUTH_CLIENT_ID`, `DASHBOARD_OAUTH_CLIENT_SECRET`, `DASHBOARD_OPERATOR_LOGIN`, and `DASHBOARD_COOKIE_KEY` are scoped to `dashboard` environment. Deploy reads the GitHub App private key from a file mount (`DASHBOARD_GITHUB_APP_KEY_FILE=/run/secrets/github-app.pem`), never an env-string fallback. The dashboard Caddy `/operator/*` same-origin route is **live and verified**: `https://dashboard.fro.bot/operator/*` routes through dashboard Caddy → VPC `10.116.0.3:9300` → gateway operator listener. Login entry: `https://dashboard.fro.bot/operator/auth/github/start` (302 → GitHub OAuth). The live route surface: `/operator/health` (200, reports `contractVersion: 1.6.0`); `/operator/auth/github/start`, `/operator/auth/github/callback`, and logout; `/operator/session` and `/operator/session/csrf` (401 unauthenticated); `/operator/repos` (401 unauthenticated); `/operator/runs` (GET/POST), `/operator/runs/:runId`, `/operator/runs/:runId/stream`, `/operator/runs/:runId/approvals`, `POST /operator/runs/:runId/approvals/:requestId/decision`, `POST /operator/runs/:runId/cancel`; `POST /operator/dispatch` (new at v0.113.2); and the push routes `/operator/push/vapid-key` and `/operator/push/subscriptions` (GET/POST/unsubscribe), which are registered conditionally and now return 401 because push is enabled — note `/operator/push/subscriptions/unsubscribe` is POST-only, so a GET returns 404 on a mounted route. Bare `/operator/` returns 404 by design; bare `/operator` (no trailing slash) does not match Caddy's `/operator/*` prefix at all, falls through to the SPA catch-all, and returns 302 — asserted in `apps/dashboard/config/Caddyfile.test.ts`. The push client ships in the deployed dashboard image: `/sw.js` serves a Workbox service worker carrying `push`, `notificationclick`, and `pushsubscriptionchange` handlers. The authenticated UI surface itself sits behind GitHub OAuth and has not been verified from outside auth. Background: `docs/plans/2026-06-18-001-feat-dashboard-operator-same-origin-plan.md`.
- Operator push is **enabled in production**: `GATEWAY_OPERATOR_PUSH_VAPID_PUBLIC_KEY`, `GATEWAY_OPERATOR_PUSH_VAPID_SUBJECT`, and `GATEWAY_OPERATOR_PUSH_VAPID_KEY_VERSION` are `gateway` Environment variables; `GATEWAY_OPERATOR_PUSH_VAPID_PRIVATE_KEY` is a `gateway` Environment secret; and `DASHBOARD_OPERATOR_PUSH_ENABLED` is a `dashboard` Environment variable set to the exact lowercase string `true`. Never record actual values. Operators do not set `GATEWAY_OPERATOR_PUSH_ENABLED` — the deploy derives it from a valid quartet, so no GitHub-stored derived flag exists. The subject is `https://dashboard.fro.bot/privacy`, a public page describing the sender: the VAPID subject is a contact URI carried in the JWT to every push service, so it must not point at an authenticated route. Verify with an unauthenticated `GET /operator/push/vapid-key` — 401 means mounted, 404 means off; a 401 proves the object-store CAS self-test passed at boot, but mount state is never re-evaluated, so it is a post-deploy check rather than a liveness probe (no post-boot signal exists — `fro-bot/agent#1637`). Browser-side delivery is not yet verified. Previous-key rotation and dedupe-window tuning remain deferred. Enablement procedure, format gates, and verification: `docs/runbooks/operator-push-enablement.md`.
...
- `VPN_SSH_KEY`, `VPN_HOST`, and `VPN_PEERS` are scoped to `vpn` environment. `VPN_PEERS` holds the peer roster JSON and is auto-synced by `vpn client add/remove`. AWS provisioning credentials (`VPN_AWS_ACCESS_KEY_ID`, `VPN_AWS_SECRET_ACCESS_KEY`, `VPN_PUBLIC_KEY`) are operator-local only — not in the `vpn` Environment and not used by deploy or status.
