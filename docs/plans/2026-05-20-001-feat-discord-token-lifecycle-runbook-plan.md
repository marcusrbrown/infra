...
- **Registration poll cadence — commit to a concrete operator-facing number**: the runbook will state, as one observable fact: "During rotation, the deploy waits ~90 seconds total for the gateway to register slash commands with Discord by default — 10 attempts × (3-second interval + 6-second per-attempt timeout) per `apps/gateway/src/deploy.ts:344-368`. If Discord returns 429 rate-limits, the wall-clock budget can stretch up to ~11 minutes (each 429 retry adds up to 60 seconds and doesn't count against the attempt budget). If the poll exhausts attempts the deploy aborts and the old checksum stays in place — safe to retry." An operator under pressure gets a clear default expectation (~90s) plus the rate-limit ceiling, without source-code archaeology. (Document-review feedback: deferring to "see deploy.ts" failed the 2am-operator framing. Fro Bot PR-review math correction: original "~3 minutes" claim was 2× the actual default; corrected here.) The separate decision of whether to align AGENTS.md's old "30-second hard gate" claim with code is deferred per Scope Boundaries — for this PR, AGENTS.md gets the same operator-observable description as the runbook.
...
- AGENTS.md `apps/gateway/AGENTS.md` includes the same operator-observable description of the registration poll cadence as the runbook (~90s default; up to ~11 min if Discord 429s; 3s interval, 6s per-attempt timeout, 10 attempts).
