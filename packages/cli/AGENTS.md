...
| Onboard new repo | `bunx @marcusrbrown/infra agent setup` | Generalized interactive wizard or `--key --repo --harness` flags; `cliproxy setup` remains a compatibility wrapper |
| Configure durable S3 storage | `bunx @marcusrbrown/infra agent storage` | Provision-first readback, non-secret `FRO_BOT_S3_*` variables, workflow/environment verification |
| Teardown durable S3 storage | `bunx @marcusrbrown/infra agent storage teardown` | Unwire S3 variables and remove repo-scoped AWS resources; retain state by default |
...
│   ├── setup.ts      → registerAgentSetup(cli) (generalized model-credential setup)
...
- **Management API**: Commands import shared helpers from `@marcusrbrown/infra-shared/cliproxy/management` (`managementHeaders`, `requestJson`, `HTTP_TIMEOUT_MS`, `parseManagementKeyList`, plus the `oauth-model-alias` helpers) for authenticated JSON requests against `/v0/management/*`. Auth header is `x-management-key` (not `Authorization: Bearer`). These primitives live in `packages/shared` so both the CLI commands and `apps/cliproxy` deploy share one source of truth (the published CLI inlines `infra-shared` at build time). Other per-file local helpers (e.g., `extractErrorMessage` 1-liners) stay local until they hit the same shared-use threshold.
- **`@clack/prompts`**: Scoped to the generalized `agent setup` flow and its `cliproxy setup` compatibility wrapper. All other commands remain non-interactive for CI/script compatibility. Import `intro`, `outro`, `text`, `select`, `confirm`, `spinner`, `note`, `isCancel`, `cancel` from `@clack/prompts`. Every prompt result MUST be checked with `isCancel()` — `cancel()` + `process.exit(0)` on cancellation. `multiselect` from `@clack/prompts` is also used in setup for the provider selection step — this is the only command that imports `multiselect`. Other commands needing operator-facing notices (e.g., `cliproxy login codex`'s anti-phishing reminder) emit plain `console.log` blocks — keeps the CI/script-safe surface uniform.
...
**Allowlist authority (`src/commands/mcp.ts`).** The `MCP_ALLOWLIST` `Set` in `mcp.ts` is the **single source of truth** for which commands appear as MCP tools. Every excluded command has a one-line reason in the JSDoc block above the allowlist (deploy-trigger and destructive policy → permanently CLI-only per the #292 decision record at `docs/brainstorms/2026-08-29-mcp-approval-gating-v2-requirements.md`; #291 and #292 are closed, interactive TTY requirement, host-machine side effect). Adding a new MCP-capturable command means adding its name to the `Set` and confirming the action threads `ctx`; adding a CLI-only command means leaving the allowlist alone and documenting the exclusion reason in the JSDoc.
...
Use the shared `ActionCtx` type from `src/lib/action-ctx.ts` — a structural subtype of `GokeExecutionContext` that captures exactly the surface actions consume. `createCapturedCtx()`, `expectCapturedToInclude()`, and `MockProcessExit` remain in `src/lib/mcp-ctx-fixture.ts` for test use only. Action bodies are exported as named functions (e.g., `gatewayStatusAction`, `cliproxyKeysListAction`) and the `.action(...)` callback delegates to them, so Tier-2 tests invoke actions directly with `createCapturedCtx()`.

**Current MCP surface = 9 read-only tools (all Mode A): the app status tools, the unified `status` tool, plus `cliproxy models`.** The `agent setup`, `agent storage`, and `agent storage teardown` commands are **source-gated out of `MCP_ALLOWLIST`** because they mutate GitHub/AWS configuration and require operator-controlled files, credentials, or environment approval. They are available through the direct CLI only. The existing mutating/secret-disclosing commands remain source-gated as well. The Mode B/C and mutation-verification guidance below documents the capture _contract_ and remains the bar for any future capturable command.
