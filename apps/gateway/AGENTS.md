...
The gateway is the Fro Bot Discord client and workspace runner — a 3-service Docker Compose stack (gateway daemon, workspace executor, mitmproxy egress filter) deployed on a dedicated DigitalOcean droplet at `gateway.fro.bot`. The upstream source is `fro-bot/agent`, pinned to `v0.46.1` in `apps/gateway/upstream.json`. There is no public HTTP surface; the gateway connects outbound to Discord and S3 only. Management happens via SSH and the `bunx @marcusrbrown/infra gateway *` CLI commands.
