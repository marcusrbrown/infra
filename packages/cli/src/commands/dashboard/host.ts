...
//
// Validates DASHBOARD_DOMAIN values before they are passed as ssh argv arguments.
...
/**
 * Validates a candidate DASHBOARD_DOMAIN value against a strict hostname allowlist.
 *
 ...
 */
export function validateDashboardHost(host: string): string {
  if (!host) {
    throw new Error('Invalid DASHBOARD_DOMAIN: value is empty')
  }

  if (!VALID_HOST_RE.test(host)) {
    // Do NOT echo the value — it may be a secret that was redirected via --key
    throw new Error(
      `Invalid DASHBOARD_DOMAIN: value is not a valid hostname (must match ${String.raw`[A-Za-z0-9][A-Za-z0-9.\-]*`})`,
    )
  }
