...
2. **Checksum gate** — SHA-256 of all secret values is computed locally. If the droplet's `/opt/gateway/.secrets-checksum` matches, the deploy exits early (no-op). The checksum file lives outside the deploy clone so `git clean -xfd` on subsequent deploys doesn't wipe it.
