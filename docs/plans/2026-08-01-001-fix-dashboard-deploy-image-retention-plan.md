...
| Fixed transaction and caller deadlines | The remote transaction is host-enforced with coreutils `timeout` at 900 seconds, sending `TERM` and escalating to `KILL` after 15 seconds; the caller watchdog is 960 seconds with an injectable shorter test seam and reports `transaction-timeout` plus the last stage without surfacing stderr. |
