...
import {resolveManagementKey} from './management'
import {formatDryRunPreview} from './preview'
import {buildApiKeyValue, cancelAndExit, ensureRepoFormat, promptGenericSecretNames, promptValue} from './prompts'
import {parseProviders, promptForModel, promptForProviders, PROVIDER_DEFAULTS, type ProviderId} from './providers'
...
import {checkFroBotWorkflow, formatWorkflowSnippet} from './workflow-analyzer'

export {formatDryRunPreview, type DryRunPreviewOptions} from './preview'
export {validateSetupOptions, verifyModelsAvailable} from './validation'
...
export interface RunSetupDeps {
  ...
  workflow?: {
    checkFroBotWorkflow: typeof checkFroBotWorkflow
  }
}
...
async function verifyWrittenNamesVisible(
  ...
  listExistingGhNames: typeof import('./gh').listExistingGhNames,
): Promise<void> {
  try {
...
export async function runSetupCommand(options: SetupOptions, deps: RunSetupDeps = {}): Promise<void> {
  ...
  const workflowChecker = deps.workflow?.checkFroBotWorkflow ?? checkFroBotWorkflow
  const mgmtKeyResolver = deps.resolveManagementKey ?? resolveManagementKey
  const commandLabel = deps.commandLabel ?? 'cliproxy setup'
    ...
    try {
      ...
      if (plan.harness === 'opencode') {
        const workflow = await gh.withGhRetry(
          ...
          async () => {
            return workflowChecker(plan.repo)
          },
          interactive,
        )
        ...
        switch (workflow.kind) {
          ...
          case 'unreachable': {
            log.warn(
              `Could not check .github/workflows/fro-bot.yaml in ${plan.repo}: ${workflow.reason}. The secrets and variables are set, but the workflow wiring was not verified. Re-run 'infra ${commandLabel}' later to confirm, or inspect the file directly.`,
            )
            break
          }
          case 'no-agent-step': {
...
export function registerSetupCommand(cli: ReturnType<typeof goke>, commandLabel: string): void {
  ...
  cli
    ...
    .example(`infra ${commandLabel} --dry-run`)
    .example('# Run the interactive onboarding wizard')
    .example(`infra ${commandLabel}`)
    .example('# Run non-interactively with an existing key (anthropic-only)')
    .example(`infra ${commandLabel} --repo owner/repo --harness opencode --key sk-existing --force`)
    .example('# Enable both providers non-interactively (requires --force and --model)')
    .example(
      `infra ${commandLabel} --repo owner/repo --harness opencode --providers anthropic,openai --model openai/gpt-5.4-mini --key sk-existing --ack-key-reuse --force`,
    )
    .action(async (options, ctx) => {
      await runSetupCommand(options, {ctx, commandLabel})
    })
}
