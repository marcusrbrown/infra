...
- `BROKER_SSH_KEY`, `BROKER_HOST`, `BROKER_MANAGEMENT_KEY`, and `CLIPROXY_MANAGEMENT_KEY` are scoped to `broker` environment. `BROKER_AUD` (the OIDC audience value) is a `broker` GitHub Environment **variable** (not a secret) — it flows at both provision time (from local `.env`) and deploy time (from `vars.BROKER_AUD` in the workflow).
