...
7. **Stale gateway-net cleanup** — Before pulling images, the deploy inspects the remote `fro-bot_gateway-net` Docker network. If it exists with the old `172.20.0.0/16` subnet (from a previous failed operator deploy), it is removed so Docker can recreate it with the correct `172.21.0.0/16` subnet. If the network is missing or already has `172.21.0.0/16`, this step is a no-op. If removal fails (e.g. active endpoints), the deploy fails closed before pull/up. This step runs after rendered-config validation and before `docker compose pull`.
8. **Pull images** — `docker compose --project-directory /opt/gateway/deploy pull` fetches the pinned GHCR images by digest. A missing or unpullable image errors here and does not fall back to an on-droplet build.
...
- **Auto-cleans stale `fro-bot_gateway-net`** — inspects the remote Docker network before `docker compose pull/up`. If it exists with the old `172.20.0.0/16` subnet, removes it so Docker can recreate it with `172.21.0.0/16`. Missing or already-correct networks are skipped. Removal failure fails the deploy closed. `GATEWAY_OPERATOR_BIND_HOST` must be in the `172.21.x.x` range (e.g. `172.21.0.2`).
