...
7. **Registration poll** — `GET /applications/{app_id}/guilds/{guild_id}/commands` on the Discord API. Polls Discord registration with ~3 min total budget, 3s interval, 6s per-attempt timeout (defaults from `apps/gateway/src/deploy.ts:344-368`). 10 attempts. 429 honors `Retry-After`. 401/403/404 abort immediately with token-sanitized errors. 5xx retries.
