...
export async function streamDashboardLogs(
  opts: StreamLogsOpts,
...
export function registerDashboardLogs(cli: ReturnType<typeof goke>): void {
  cli
    .command('dashboard logs [service]', 'Stream logs from a dashboard service via SSH and docker compose.')
    ...
    .action(async (service, options) => {
      const targetService = (service as string | undefined) ?? 'dashboard'
      ...
      const result = await streamDashboardLogs({host, service: targetService, tail, allowCi})
