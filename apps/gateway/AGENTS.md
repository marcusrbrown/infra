...
The gateway is the Fro Bot Discord client and workspace runner — a 3-service Docker Compose stack (gateway daemon, workspace executor, mitmproxy egress filter) deployed on a dedicated DigitalOcean droplet at `gateway.fro.bot`. The upstream source is `fro-bot/agent`, pinned to `v0.113.2` in `apps/gateway/upstream.json`. v0.113.2's startup config parser (`packages/gateway/src/config.ts`) is byte-identical to v0.93.1 — no new boot-required secrets. It adds `POST /operator/dispatch` (operator contract stays 1.6.0) and caps `POST /operator/runs`'s `idempotencyKey` at 256 characters (400 above that). The workspace image bumps to Node `24.21.0-alpine`, embedded OpenCode `1.18.30+harness.7c479429`, and Systematic `3.18.4`, and now bakes `OPENCODE_EXPERIMENTAL_DISABLE_FILEWATCHER=true`. Do not override it back on. The only state that watcher refreshes is OpenCode's per-directory cached current branch, read solely by `GET /vcs` and branch-mode `GET /vcs/diff` — neither of which the gateway mention loop calls, so the staleness is inert here. Two corrections to the upstream note: a stale value skews branch-mode diff results, not just a displayed branch name; and clearing this flag does not enable recursive working-tree watching, which is a separate opt-in (`OPENCODE_EXPERIMENTAL_FILEWATCHER`). If reused checkouts under `/workspace/repos` drift behind GitHub, that is a checkout-update policy problem — `/clone` returns `409 repo-exists` and never fetches or switches branches — and no amount of file watching fixes it. `/fro-bot dispatch` needs the GitHub App to hold `Actions: read and write`; that permission is granted on the Fro Bot Agent App and accepted by the installations that use it. The v0.93.1 deploy-consumed contract (`deploy/compose.yaml`, `deploy/gateway.Dockerfile`, `deploy/workspace.Dockerfile`, `deploy/mitmproxy/allowlist.py`, `deploy/workspace-entrypoint.sh`) is byte-identical to v0.88.0 — no secret/env/build-arg/image/entrypoint wiring changes. v0.88.0 adds native always-on session tools and removes the OpenCode SDK age-gate exclusion; its deploy-consumed contract is byte-identical to v0.86.0. v0.79.1 bumps the upstream mitmproxy image to 12.2.3 and the workspace image's OpenCode/Systematic build args to 1.17.11+harness.bf0e9bed / 2.32.1; no operator route, secret/env, allowlist script, or runtime entrypoint contract changes are required by this repo's deploy path. v0.78.0 adds `GET /operator/runs` (read-only session-gated operator run index) and fixes operator listing fanout dedupe; operator contract docs bumped to 1.5.0 — behavioral, no new required secrets, config contract identical to v0.77.0. v0.77.0 mounts the operator launch route (POST /operator/runs) and adds image-level operator route-registration smoke — behavioral, no new required secrets, config contract identical to v0.76.0. v0.76.0 adds web tool-approval support (gateway approval contract 1.4.0) — behavioral, no new required secrets. v0.75.0 is an intermediate release on the same approval-contract line — no new required secrets. v0.74.0 fixes the operator OAuth callback to redirect to the validated return_to path after minting the session (completes the operator-session re-auth round-trip) — behavioral, no new required secrets. v0.73.0 continues the operator-web work: operator session/OAuth/CSRF/allowlist auth surface + run-stream plumbing the dashboard consumes — behavioral on existing `GATEWAY_OPERATOR_*` config, no new required secrets. v0.72.0 adds the authenticated SSE run-stream (operator), server-side operator token retention, and metadata/repos.yaml redaction on operator surfaces — all behavioral on existing `GATEWAY_OPERATOR_*` config, no new required secrets. v0.69.0 adds the operator auth/session/config contract: GitHub OAuth client ID/secret, CSRF secret, and allowlist — all materialized as `_FILE` mounts — on top of the v0.66.0 listener topology. v0.66.0 adds the operator listener topology contract (`fro-bot/agent#931`): `GATEWAY_OPERATOR_BIND_HOST`, `GATEWAY_OPERATOR_BIND_PORT`, and `GATEWAY_OPERATOR_PUBLIC_ORIGIN` enable the operator listener when all three are set. At that point the operator route surface was `GET /operator/health`. v0.64.3 bumped the harness OpenCode build with no required-secret or topology change vs v0.63.0. v0.60.0 introduced a serial per-channel run queue (one active run per Discord channel at a time) and the `/fro-bot force-release-lock` slash command for operator-driven lock release. v0.59.0 added a live-status message + typing indicator for mention runs (`GATEWAY_STATUS_MODE`, defaults to `live-status`, no infra wiring needed). v0.57.0 added the `daily_digest` presence/announce event, reusing the existing `/v1/announce` HMAC webhook ingress (`GATEWAY_WEBHOOK_SECRET` + `GATEWAY_PRESENCE_CHANNEL_ID` already configured). `WORKSPACE_EGRESS_HOSTS` is consumed by `deploy/mitmproxy/allowlist.py` (comma-separated exact hosts the sandboxed workspace may reach through the mitmproxy egress proxy; fail-closed if empty). The deploy emits `WORKSPACE_EGRESS_HOSTS=cliproxy.fro.bot,models.dev` so the workspace OpenCode can reach the cliproxy endpoint and fetch its model catalog from `models.dev` at startup. The workspace executor exposes `/healthz` + `/clone` on `:9100` and an OpenCode bearer proxy on `:9200`, all on the internal `sandbox-net` with no host ports, enabling both `/fro-bot add-project` repo cloning and the `@fro-bot` mention loop. `GATEWAY_WEBHOOK_SECRET` and `GATEWAY_PRESENCE_CHANNEL_ID` are `readOptionalSecret` (opt-in) in the upstream daemon — set both to enable the announce/presence webhook (see [Announce/presence webhook](#announcepresence-webhook) below), or leave both unset to keep the gateway outbound-only. The deploy materializes GitHub App credentials (`github-app-id`, `github-app-private-key`) from `GH_APP_ID` / `GH_APP_PRIVATE_KEY` environment secrets.
...
Both build steps pin a `docker/setup-buildx-action` builder to a fixed `moby/buildkit` image, set `SOURCE_DATE_EPOCH` as a build-arg, and export with `rewrite-timestamp=true` — this makes config/history/index timestamps and in-layer tar timestamps deterministic for a given upstream ref. The upstream Dockerfile's two unpinned `apk add --no-cache` lines (`curl unzip`, `netcat-openbsd`) remain the limit on guaranteed bit-reproducibility — that's upstream's to pin. To check it, compare the digests of two published tags sharing an upstream ref prefix (`<ref>-<sha>`): identical digests mean that ref reproduces. Layers map to instructions via the image config `history` (skipping `empty_layer` entries), so a drifting index names the instruction that caused it; equal size with a different digest is timestamp drift, not content.
...
1. **Gateway-side liveness probe:** Probe the listener directly via the droplet-local gateway-net address:
   ...
   **⚠ Liveness probe warning:** Do **not** use `https://gateway.fro.bot/operator/health` as a liveness probe — v0.69.0 adds a global forwarded-header guard that returns `400 {"error":"bad request"}` for any request where `X-Forwarded-Host` does not match the `PUBLIC_ORIGIN` host (`dashboard.fro.bot`); the public Caddy route forwards `Host: gateway.fro.bot`, so it returns 400 by design with no trusted-proxy config knob. The gateway-side liveness probe is the droplet-local direct probe above. The dashboard same-origin path is **live**: `https://dashboard.fro.bot/operator/health` → `200 {"ok":true}` is a valid external probe. The OAuth callback URL registration uses the dashboard origin (`https://dashboard.fro.bot/operator/auth/github/callback`) — this is a GitHub OAuth App setting, not a live HTTP probe target.

2. **Auth gate coarse check:** `GET https://gateway.fro.bot/operator/` returns `400 {"error":"bad request"}` by design — the v0.69.0 forwarded-header guard rejects it because the forwarded host (`gateway.fro.bot`) does not match the `PUBLIC_ORIGIN` host (`dashboard.fro.bot`). Do not use `gateway.fro.bot/operator/` as an auth probe. The auth-gate / OAuth-redirect check is meaningful through the live same-origin path: `curl -sI https://dashboard.fro.bot/operator/auth/github/start` returns `302` to GitHub OAuth. A full browser round-trip returns the operator identity JSON `{"githubUserId":<operator-id>,"login":<login>}` from the callback when the GitHub user is on `GATEWAY_OPERATOR_ALLOWLIST`.
...
All four absent is disabled. Any partial set hard-fails before SSH, spawn, secret materialization, or
remote writes and names every missing key. A complete set must pass strict validation: the public key
is unpadded base64url decoding to exactly 65 bytes with a `0x04` first byte; the private key is
unpadded base64url decoding to exactly 32 bytes; the subject is nonblank and uses `mailto:` or
`https:`; and the key version is a positive integer string.

Operators never set `GATEWAY_OPERATOR_PUSH_ENABLED`. The deploy derives
`GATEWAY_OPERATOR_PUSH_ENABLED=true` only from a complete valid quartet and emits no false line. The
four files are materialized through the existing SSH-stdin path only; the private key never appears in
...
followed by an absent quartet, the bounded cleanup deletes exactly the four known files under
`/opt/gateway/deploy/secrets/` before current secret materialization; the new override omits the VAPID
mounts and derived flag, and the checksum path recreates the gateway disabled. A never-enabled absent
quartet remains the ordinary no-file path.

The authenticated `GET /operator/push/vapid-key` route returns only `{publicKey, keyVersion}` when
enabled. It returns 404 when push is disabled or the operator guard denies the request.

### Disabled-state verification

Verify names and status only: the four gateway VAPID secret/variable names remain absent, the dashboard
push variable remains absent or false, and no independently forwarded gateway enable setting exists.
There is no GitHub-stored derived gateway flag to inspect. Never retrieve, print, copy, or compare
secret contents.
...
**Current state:** The operator listener is **enabled in production**. `GATEWAY_OPERATOR_BIND_HOST=172.21.0.2`, `GATEWAY_OPERATOR_BIND_PORT=9300`, and `GATEWAY_OPERATOR_PUBLIC_ORIGIN=https://dashboard.fro.bot` are set in the `gateway` GitHub Environment. Deploy run [27740787921](https://github.com/marcusrbrown/infra/actions/runs/27740787921) succeeded; operator listener liveness is confirmed via the droplet-local direct probe (`curl -sf http://172.21.0.2:9300/operator/health` → `200 {"ok":true}`). The public `https://gateway.fro.bot/operator/health` route returns 400 by design (v0.69.0 forwarded-header guard — `X-Forwarded-Host: gateway.fro.bot` does not match `PUBLIC_ORIGIN` host `dashboard.fro.bot`). Operator auth is **live**: all four auth/config secrets (`GATEWAY_OPERATOR_GITHUB_CLIENT_ID`, `GATEWAY_OPERATOR_GITHUB_CLIENT_SECRET`, `GATEWAY_OPERATOR_CSRF_SECRET`, `GATEWAY_OPERATOR_ALLOWLIST`) are seeded in the `gateway` GitHub Environment and materialized as `_FILE` mounts. The dashboard Caddy `/operator/*` same-origin route is **live**: `https://dashboard.fro.bot/operator/*` routes through dashboard Caddy → VPC `10.116.0.3:9300` → gateway operator listener. End-to-end auth is verified: `https://dashboard.fro.bot/operator/auth/github/start` returns `302` to GitHub OAuth; completing the flow returns the operator identity JSON for an allowlisted user. The push client ships in the deployed dashboard image: `https://dashboard.fro.bot/sw.js` serves a Workbox service worker carrying `push`, `notificationclick`, and `pushsubscriptionchange` handlers. The authenticated UI surface itself sits behind GitHub OAuth and has not been verified from outside auth. Background: `docs/plans/2026-06-18-001-feat-dashboard-operator-same-origin-plan.md`.
